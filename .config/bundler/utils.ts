...
export async function getEntries() {
  ...
  const plugins = await Promise.all(
    pluginsJson.map((pluginJson) => {
      ...
      return glob(`${folder.replace(/\\/g, '/')}/module.{ts,tsx,js,jsx}`, {
        absolute: true,
  ...
  return plugins.reduce<Record<string, string>>((result, modules) => {
    return modules.reduce((innerResult, module) => {
      ...
      const relFromSrc = path.relative(path.resolve(process.cwd(), SOURCE_DIR), pluginPath);
      const pluginName = relFromSrc.split(path.sep).join('/');
      const entryName = pluginName ? `${pluginName}/module` : 'module';
